-- Clean setup script with policy existence check
-- This script safely handles existing policies and recreates the database structure
...
DO $$
BEGIN
    IF policy_exists('Users can view own profile', 'users') THEN
        DROP POLICY "Users can view own profile" ON users;
    END IF;
    
    IF policy_exists('Users can update own profile', 'users') THEN
        DROP POLICY "Users can update own profile" ON users;
    END IF;
    
    IF policy_exists('Users can insert own profile', 'users') THEN
        DROP POLICY "Users can insert own profile" ON users;
    END IF;
    
    IF policy_exists('Users can view own projects', 'user_projects') THEN
        DROP POLICY "Users can view own projects" ON user_projects;
    END IF;
    
    IF policy_exists('Users can manage own projects', 'user_projects') THEN
        DROP POLICY "Users can manage own projects" ON user_projects;
    END IF;
    
    IF policy_exists('Users can insert own projects', 'user_projects') THEN
        DROP POLICY "Users can insert own projects" ON user_projects;
    END IF;
    
    IF policy_exists('Users can update own projects', 'user_projects') THEN
        DROP POLICY "Users can update own projects" ON user_projects;
    END IF;
    
    IF policy_exists('Users can delete own projects', 'user_projects') THEN
        DROP POLICY "Users can delete own projects" ON user_projects;
    END IF;
    
    IF policy_exists('Users can view own calculations', 'solar_calculations') THEN
        DROP POLICY "Users can view own calculations" ON solar_calculations;
    END IF;
    
    IF policy_exists('Users can manage own calculations', 'solar_calculations') THEN
        DROP POLICY "Users can manage own calculations" ON solar_calculations;
    END IF;
    
    IF policy_exists('Users can insert own calculations', 'solar_calculations') THEN
        DROP POLICY "Users can insert own calculations" ON solar_calculations;
    END IF;
    
    IF policy_exists('Users can update own calculations', 'solar_calculations') THEN
        DROP POLICY "Users can update own calculations" ON solar_calculations;
    END IF;
    
    IF policy_exists('Users can delete own calculations', 'solar_calculations') THEN
        DROP POLICY "Users can delete own calculations" ON solar_calculations;
    END IF;
END $$;

-- Drop the helper function
DROP FUNCTION policy_exists(text, text);
