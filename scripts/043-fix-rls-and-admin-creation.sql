-- Fix RLS policies and admin creation for Solar Grind V2
-- This script addresses the RLS policy violation when creating demo admin
...
CREATE POLICY "Allow all operations for authenticated users" ON public.user_projects
    FOR ALL USING (true) WITH CHECK (true);

CREATE POLICY "Allow all operations for authenticated users" ON public.solar_calculations
    FOR ALL USING (true) WITH CHECK (true);

CREATE POLICY "Allow all operations for authenticated users" ON public.usage_tracking
    FOR ALL USING (true) WITH CHECK (true);
...
BEGIN
    ...
    -- Insert or update the demo admin user (bypasses RLS due to SECURITY DEFINER)
...
BEGIN
    ...
    -- Insert the user (bypasses RLS due to SECURITY DEFINER)
    INSERT INTO public.users (
        id,
        email,
        name,
...
DO $$
BEGIN
    RAISE NOTICE 'Database setup complete with permissive RLS policies!';
